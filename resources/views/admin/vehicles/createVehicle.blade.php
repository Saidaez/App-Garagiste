<div id="createModal" class="modal fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50" data-aos="fade-down">
    <!-- Modal content -->
    <div id="modal-content" class="modal-content bg-white dark:bg-gray-800 shadow-xl px-1 rounded-lg">
        ...
        <div class="p-6 text-gray-900 dark:text-gray-100">
            <h2 class="block text-center mb-4">Create Vehicle</h2>
            <form action="{{ route('vehicles.store') }}" method="POST" id="createForm">
                @csrf
                <div class="mb-4">
                    <x-input-label for="make" :value="__('Make')" />
                    <x-text-input id="make" class="form-control" type="text" name="make" required />
                </div>
                <div class="mb-4">
                    <x-input-label for="model" :value="__('Model')" />
                    <x-text-input id="model" class="form-control" type="text" name="model" required />
                </div>
                <div class="mb-4">
                    <x-input-label for="fuelType" :value="__('Fuel Type')" />
                    <x-text-input id="fuelType" class="form-control" type="text" name="fuelType" required />
                </div>
                <div class="mb-4">
                    <x-input-label for="registration" :value="__('Registration')" />
                    <x-text-input id="registration" class="form-control" type="text" name="registration" required />
                </div>
                <div class="mb-4">
